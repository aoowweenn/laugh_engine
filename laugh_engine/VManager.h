...
struct SubpassCreateInfo {
  ...
  std::vector<VkAttachmentReference>
      resolveAttachmentRefs; // must have the same size as @colorAttachmentRefs
...
class VManager {
public:
  VManager(void *app, GLFWkeyfun keyfun = nullptr,
           GLFWmousebuttonfun mousebuttonfun = nullptr,
           GLFWcursorposfun cursorposfun = nullptr,
           GLFWscrollfun scrollfun = nullptr,
           GLFWwindowsizefun windowsizefun = nullptr, uint32_t winWidth = 1920,
           uint32_t winHeight = 1080, const std::string &winTitle = "",
           const VkPhysicalDeviceFeatures &enabledFeatures = {})
      : m_instance{m_enableValidationLayers,
                   {"VK_LAYER_LUNARG_standard_validation"},
                   VWindow::getRequiredExtensions()},
        m_window{m_instance, winWidth,     winHeight,      winTitle,
                 app,        keyfun,       mousebuttonfun, cursorposfun,
                 scrollfun,  windowsizefun},
        m_device{m_enableValidationLayers,
                 {"VK_LAYER_LUNARG_standard_validation"},
                 m_instance,
                 m_window,
  ...
  void renderPassAddAttachment(
      VkFormat format, VkImageLayout initLayout, VkImageLayout finalLayout,
    ...
    VkAttachmentDescription &attachment =
        m_curRenderPassInfo.attachmentDescs.back();
  ...
  void subpassAddColorAttachmentReference(uint32_t attachmentIdx,
                                          VkImageLayout layout) {
    m_pCurSubpassInfo->colorAttachmentRefs.push_back({});
    VkAttachmentReference &attachmentRef =
        m_pCurSubpassInfo->colorAttachmentRefs.back();

    attachmentRef.attachment = attachmentIdx;
    attachmentRef.layout = layout;
  }

  void subpassAddResolveAttachmentReference(uint32_t attachmentIdx,
                                            VkImageLayout layout) {
    m_pCurSubpassInfo->resolveAttachmentRefs.push_back({});
    VkAttachmentReference &attachmentRef =
        m_pCurSubpassInfo->resolveAttachmentRefs.back();

    attachmentRef.attachment = attachmentIdx;
    attachmentRef.layout = layout;
  }

  void subpassAddDepthAttachmentReference(uint32_t attachmentIdx,
                                          VkImageLayout layout) {
    ...
    VkAttachmentReference &attachmentRef =
        m_pCurSubpassInfo->depthAttachmentRefs.back();

    attachmentRef.attachment = attachmentIdx;
    attachmentRef.layout = layout;
  }

  void subpassAddInputAttachmentReference(uint32_t attachmentIdx,
                                          VkImageLayout layout) {
    m_pCurSubpassInfo->inputAttachmentRefs.push_back({});
    VkAttachmentReference &attachmentRef =
        m_pCurSubpassInfo->inputAttachmentRefs.back();
  ...
  void endDescribeSubpass(
    ...
    if (m_pCurSubpassInfo->colorAttachmentRefs.size() > 0) {
      subpass.colorAttachmentCount =
          static_cast<uint32_t>(m_pCurSubpassInfo->colorAttachmentRefs.size());
      subpass.pColorAttachments = m_pCurSubpassInfo->colorAttachmentRefs.data();
    }

    if (m_pCurSubpassInfo->resolveAttachmentRefs.size() > 0) {
      if (m_pCurSubpassInfo->resolveAttachmentRefs.size() !=
          m_pCurSubpassInfo->colorAttachmentRefs.size()) {
        throw std::runtime_error("resolve attachment count must either be 0 or "
                                 "the same as color attachment count");
      }
      subpass.pResolveAttachments =
          m_pCurSubpassInfo->resolveAttachmentRefs.data();
    }

    if (m_pCurSubpassInfo->depthAttachmentRefs.size() > 0) {
      subpass.pDepthStencilAttachment =
          m_pCurSubpassInfo->depthAttachmentRefs.data();
    }

    if (m_pCurSubpassInfo->inputAttachmentRefs.size() > 0) {
      subpass.inputAttachmentCount =
          static_cast<uint32_t>(m_pCurSubpassInfo->inputAttachmentRefs.size());
      subpass.pInputAttachments = m_pCurSubpassInfo->inputAttachmentRefs.data();
    }

    if (m_pCurSubpassInfo->preserveAttachmentRefs.size() > 0) {
      subpass.preserveAttachmentCount = static_cast<uint32_t>(
          m_pCurSubpassInfo->preserveAttachmentRefs.size());
      subpass.pPreserveAttachments =
          m_pCurSubpassInfo->preserveAttachmentRefs.data();
    }
  ...
  void renderPassAddSubpassDependency(uint32_t srcSubpass, uint32_t dstSubpass,
    ...
    VkSubpassDependency &dependency =
        m_curRenderPassInfo.subpassDependencies.back();
  ...
  uint32_t endCreateRenderPass() {
    ...
    renderPassInfo.attachmentCount =
        static_cast<uint32_t>(m_curRenderPassInfo.attachmentDescs.size());
    renderPassInfo.pAttachments = m_curRenderPassInfo.attachmentDescs.data();
    renderPassInfo.subpassCount =
        static_cast<uint32_t>(m_curRenderPassInfo.subpassDescs.size());
    renderPassInfo.pSubpasses = m_curRenderPassInfo.subpassDescs.data();
    renderPassInfo.dependencyCount =
        static_cast<uint32_t>(m_curRenderPassInfo.subpassDependencies.size());
    renderPassInfo.pDependencies =
        m_curRenderPassInfo.subpassDependencies.data();

    if (vkCreateRenderPass(m_device, &renderPassInfo, nullptr,
                           m_renderPasses[m_curRenderPassName].replace()) !=
        VK_SUCCESS) {
  ...
  void destroyRenderPass(uint32_t renderPassName) {
    ...
    assert(std::find(m_availableRenderPassNames.begin(),
                     m_availableRenderPassNames.end(),
                     renderPassName) == m_availableRenderPassNames.end());
  ...
  void beginCreateDescriptorSetLayout() {
    ...
    m_descriptorSetLayouts[m_curSetLayoutName] =
        VDeleter<VkDescriptorSetLayout>{m_device, vkDestroyDescriptorSetLayout};
  }

  void
  setLayoutAddBinding(uint32_t bindingPoint, VkDescriptorType type,
                      VkShaderStageFlags shaderStages, uint32_t count = 1,
                      const std::vector<uint32_t> &immutableSamplerNames = {}) {
    ...
    std::vector<VkSampler> &immutableSamplers =
        m_curSetLayoutInfo.immutableSamplers.back();
  ...
  uint32_t endCreateDescriptorSetLayout() {
    ...
    layoutInfo.bindingCount =
        static_cast<uint32_t>(m_curSetLayoutInfo.bindings.size());
    layoutInfo.pBindings = m_curSetLayoutInfo.bindings.data();

    if (vkCreateDescriptorSetLayout(
            m_device, &layoutInfo, nullptr,
            m_descriptorSetLayouts[m_curSetLayoutName].replace()) !=
        VK_SUCCESS) {
  ...
  void pipelineLayoutAddDescriptorSetLayouts(
      const std::vector<uint32_t> &setLayoutNames) {
  ...
  void pipelineLayoutAddPushConstantRange(uint32_t offset, uint32_t size,
                                          VkShaderStageFlags shaderStages) {
    m_curPipelineLayoutInfo.pushConstantRanges.push_back({});
    VkPushConstantRange &psr =
        m_curPipelineLayoutInfo.pushConstantRanges.back();
  ...
  uint32_t endCreatePipelineLayout() {
    if (m_curPipelineLayoutInfo.descriptorSetLayouts.size() == 0) {
      throw std::runtime_error(
          "pipeline layout contains at least one descriptor set layout");
    }

    VkPipelineLayoutCreateInfo info = {};

    info.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    info.setLayoutCount = static_cast<uint32_t>(
        m_curPipelineLayoutInfo.descriptorSetLayouts.size());
    std::vector<VkDescriptorSetLayout> setLayouts(
        m_curPipelineLayoutInfo.descriptorSetLayouts.begin(),
        m_curPipelineLayoutInfo.descriptorSetLayouts.end());
    info.pSetLayouts = setLayouts.data();
    info.pushConstantRangeCount = static_cast<uint32_t>(
        m_curPipelineLayoutInfo.pushConstantRanges.size());
    info.pPushConstantRanges =
        m_curPipelineLayoutInfo.pushConstantRanges.data();

    if (vkCreatePipelineLayout(
            m_device, &info, nullptr,
            m_pipelineLayouts[m_curPipelineLayoutName].replace()) !=
        VK_SUCCESS) {
  ...
  void destroyPipelineLayout(uint32_t pipelineLayoutName) {
    ...
    assert(std::find(m_availablePipelineLayoutNames.begin(),
                     m_availablePipelineLayoutNames.end(),
                     pipelineLayoutName) ==
           m_availablePipelineLayoutNames.end());
  ...
  void beginCreateGraphicsPipeline(
      uint32_t layoutName, uint32_t renderPassName, uint32_t subpassIdx,
      uint32_t basePipelineName = std::numeric_limits<uint32_t>::max(),
    ...
    if (flags & VK_PIPELINE_CREATE_DERIVATIVE_BIT) {
      if (basePipelineName < m_pipelines.size() &&
          std::find(m_availablePipelineNames.begin(),
                    m_availablePipelineNames.end(),
                    basePipelineName) == m_availablePipelineNames.end()) {
  ...
  void
  graphicsPipelineAddShaderStage(VkShaderStageFlagBits stage,
    ...
    VkPipelineShaderStageCreateInfo &shaderStageInfo =
        m_curGraphicsPipelineInfo.shaderStages.back();
    ...
    case VK_SHADER_STAGE_VERTEX_BIT:
      m_curGraphicsPipelineInfo.vertShaderModule =
          VDeleter<VkShaderModule>{m_device, vkDestroyShaderModule};
      createShaderModule(m_curGraphicsPipelineInfo.vertShaderModule, m_device,
                         shaderByteCode);
      shaderStageInfo.module = m_curGraphicsPipelineInfo.vertShaderModule;
      break;
    case VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT:
      m_curGraphicsPipelineInfo.hullShaderModule =
          VDeleter<VkShaderModule>{m_device, vkDestroyShaderModule};
      createShaderModule(m_curGraphicsPipelineInfo.hullShaderModule, m_device,
                         shaderByteCode);
      shaderStageInfo.module = m_curGraphicsPipelineInfo.hullShaderModule;
      break;
    case VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT:
      m_curGraphicsPipelineInfo.domainShaderModule =
          VDeleter<VkShaderModule>{m_device, vkDestroyShaderModule};
      createShaderModule(m_curGraphicsPipelineInfo.domainShaderModule, m_device,
                         shaderByteCode);
      shaderStageInfo.module = m_curGraphicsPipelineInfo.domainShaderModule;
      break;
    case VK_SHADER_STAGE_GEOMETRY_BIT:
      m_curGraphicsPipelineInfo.geomShaderModule =
          VDeleter<VkShaderModule>{m_device, vkDestroyShaderModule};
      createShaderModule(m_curGraphicsPipelineInfo.geomShaderModule, m_device,
                         shaderByteCode);
      shaderStageInfo.module = m_curGraphicsPipelineInfo.geomShaderModule;
      break;
    case VK_SHADER_STAGE_FRAGMENT_BIT:
      m_curGraphicsPipelineInfo.fragShaderModule =
          VDeleter<VkShaderModule>{m_device, vkDestroyShaderModule};
      createShaderModule(m_curGraphicsPipelineInfo.fragShaderModule, m_device,
                         shaderByteCode);
    ...
    m_curGraphicsPipelineInfo.pipelineInfo.stageCount =
        static_cast<uint32_t>(m_curGraphicsPipelineInfo.shaderStages.size());
    m_curGraphicsPipelineInfo.pipelineInfo.pStages =
        m_curGraphicsPipelineInfo.shaderStages.data();
  }
  ...
  void graphicsPipelineAddSpecializationConstant(VkShaderStageFlagBits stage,
    ...
    auto checkShaderModule = [](const VDeleter<VkShaderModule> &module) {
      if (!module.isvalid()) {
        throw std::runtime_error("try to add specialization constant to shader "
                                 "stage but the stage doesn't exist");
      }
    };
    ...
    if (size == 0)
      throw std::invalid_argument("size need to be greater than 0");
  ...
  void graphicsPipelineAddAttributeDescription(uint32_t location,
                                               uint32_t binding,
                                               VkFormat format,
                                               uint32_t offset) {
  ...
  void graphicsPipelineConfigureInputAssembly(
      VkPrimitiveTopology topology, VkBool32 enablePrimitiveRestart = VK_FALSE,
  ...
  void graphicsPipelineConfigureTessellationState(
      uint32_t numCPsPerPatch,
  ...
  void graphicsPipelineAddViewportAndScissor(
      float viewportX, float viewportY, float viewportWidth,
      float viewportHeight, float minDepth = 0.f, float maxDepth = 1.f,
      int32_t scissorX = 0, int32_t scissorY = 0, uint32_t scissorWidth = 0,
      uint32_t scissorHeight = 0, bool coverEntireViewport = true) {
    if (viewportWidth < 0.f || viewportHeight < 0.f || scissorWidth < 0 ||
        scissorHeight < 0) {
      throw std::invalid_argument(
          "invalid arguments to graphicsPipelineAddViewportAndScissor");
    }
    ...
    info.viewportCount =
        static_cast<uint32_t>(m_curGraphicsPipelineInfo.viewports.size());
    info.pViewports = m_curGraphicsPipelineInfo.viewports.data();
    info.scissorCount =
        static_cast<uint32_t>(m_curGraphicsPipelineInfo.scissors.size());
    info.pScissors = m_curGraphicsPipelineInfo.scissors.data();
  }

  void graphicsPipelineConfigureRasterizer(
      VkPolygonMode polygonMode, VkCullModeFlags cullMode,
      VkFrontFace frontFace, float lineWidth = 1.f,
      VkBool32 depthBiasEnable = VK_FALSE, float depthBiasConstantFactor = 0.f,
      float depthBiasSlopeFactor = 1.f, VkBool32 depthClampEnable = VK_FALSE,
      float depthBiasClamp = 1.f, VkBool32 rasterizerDiscardEnable = VK_FALSE,
  ...
  void graphicsPipelineConfigureMultisampleState(
      VkSampleCountFlagBits sampleCount, VkBool32 perSampleShading = VK_FALSE,
      float minSampleShadingFraction = 1.f,
  ...
  void graphicsPipelineConfigureDepthState(
  ...
  void graphicsPipelineConfigureStencilState(
      VkBool32 stencilTestEnable, VkStencilOp failOp, VkStencilOp passOp,
      VkStencilOp depthFailOp, VkCompareOp compareOp, uint32_t reference,
      uint32_t compareMask = 0xffffffff, uint32_t writeMask = 0xffffffff,
      bool frontOp = true) {
    auto &info = m_curGraphicsPipelineInfo.depthStencilInfo;
  ...
  // If logic op is enabled, blending is automatically disabled on all color
  // attachments
  // Logic op can only be applied to sint, uint, snorm, or unorm attachments.
  // For floating point attachments, fragment outputs are pass through without
  // modification.
  void graphicsPipelineConfigureLogicOp(VkBool32 logicOpEnable,
                                        VkLogicOp logicOp) {
  ...
  void graphicsPipeLineAddColorBlendAttachment(
      VkBool32 blendEnable,
      VkBlendFactor srcColorBlendFactor = VK_BLEND_FACTOR_SRC_ALPHA,
      VkBlendFactor dstColorBlendFactor = VK_BLEND_FACTOR_ZERO,
      VkBlendOp colorBlendOp = VK_BLEND_OP_ADD, bool alphaSameAsColor = true,
    ...
    VkPipelineColorBlendAttachmentState &info =
        m_curGraphicsPipelineInfo.colorBlendAttachmentStates.back();
    ...
    info.srcAlphaBlendFactor =
        alphaSameAsColor ? srcColorBlendFactor : srcAlphaBlendFactor;
    info.dstAlphaBlendFactor =
        alphaSameAsColor ? dstColorBlendFactor : dstAlphaBlendFactor;
    ...
    m_curGraphicsPipelineInfo.colorBlendInfo.attachmentCount =
        static_cast<uint32_t>(
            m_curGraphicsPipelineInfo.colorBlendAttachmentStates.size());
  ...
  void graphicsPipelineAddDynamicState(VkDynamicState dynamicState) {
    ...
    m_curGraphicsPipelineInfo.dynamicStateInfo.dynamicStateCount =
        static_cast<uint32_t>(m_curGraphicsPipelineInfo.dynamicStates.size());
    m_curGraphicsPipelineInfo.dynamicStateInfo.pDynamicStates =
        m_curGraphicsPipelineInfo.dynamicStates.data();
  }
  ...
  uint32_t endCreateGraphicsPipeline() {
    ...
    if (vkCreateGraphicsPipelines(m_device, m_pipelineCache, 1, &info, nullptr,
                                  m_pipelines[m_curPipelineName].replace()) !=
        VK_SUCCESS) {
  ...
  void beginCreateComputePipeline(
      uint32_t layoutName,
      uint32_t basePipelineName = std::numeric_limits<uint32_t>::max(),
      VkPipelineCreateFlags flags = 0) {
  ...
  void
  computePipelineAddShaderStage(const std::string &spvFileName,
                                VkPipelineShaderStageCreateFlags flags = 0) {
    auto shaderByteCode = readFile(spvFileName);
    m_curComputePipelineInfo.computeShaderModule =
        VDeleter<VkShaderModule>{m_device, vkDestroyShaderModule};
    createShaderModule(m_curComputePipelineInfo.computeShaderModule, m_device,
                       shaderByteCode);
  ...
  void computePipelineAddSpecializationConstant(uint32_t constantID,
                                                uint32_t offset, uint32_t size,
                                                void *srcData) {
    if (!srcData)
      throw std::invalid_argument("data cannot be null");
    if (size == 0)
      throw std::invalid_argument("size need to be greater than 0");
    if (!m_curComputePipelineInfo.computeShaderModule.isvalid())
      throw std::runtime_error(
          "Cannot add specialization data to empty compute shader stage");

    auto &specializationInfo =
        m_curComputePipelineInfo.computeSpecializationInfo;
    auto &mapEntries = m_curComputePipelineInfo.computeSpecializationMapEntries;
    mapEntries.push_back({});
    auto &mapEntry = mapEntries.back();
    auto &specializationData =
        m_curComputePipelineInfo.computeSpecializationData;
    ...
    m_curComputePipelineInfo.pipelineInfo.stage.pSpecializationInfo =
        &specializationInfo;
  }

  uint32_t endCreateComputePipeline() {
    if (vkCreateComputePipelines(
            m_device, m_pipelineCache, 1,
            &m_curComputePipelineInfo.pipelineInfo, nullptr,
            m_pipelines[m_curPipelineName].replace()) != VK_SUCCESS) {
  ...
  void destroyPipeline(uint32_t pipelineName) {
    ...
    assert(std::find(m_availablePipelineNames.begin(),
                     m_availablePipelineNames.end(),
                     pipelineName) == m_availablePipelineNames.end());
  ...
  // --- Image related ---
  uint32_t
  createImage2D(uint32_t width, uint32_t height, VkFormat format,
    ...
    m_images.at(imageName).initAs2DImage(width, height, format, usage, memProps,
                                         mipLevels, arrayLayers, sampleCount,
  ...
  uint32_t
  createImageCube(uint32_t width, uint32_t height, VkFormat format,
                  VkImageUsageFlags usage, VkMemoryPropertyFlags memProps,
    ...
    m_images.at(imageName).initAsCubeImage(width, height, format, usage,
                                           memProps, mipLevels, initialLayout,
                                           tiling);
  ...
  void destroyImage(uint32_t imageName) {
    ...
    assert(std::find(m_availableImageNames.begin(), m_availableImageNames.end(),
                     imageName) == m_availableImageNames.end());
  ...
  // --- Image view related ---
  uint32_t createImageView(uint32_t imageName, VkImageViewType viewType,
    ...
    m_imageViews.at(viewName).init(imageName, viewType, aspectMask,
                                   baseMipLevel, levelCount, baseArrayLayer,
                                   layerCount, componentMapping, flags);

    return viewName;
  }

  uint32_t createImageViewCube(uint32_t imageName,
                               VkImageAspectFlags aspectMask,
                               ...
                               VkComponentMapping componentMapping = {},
                               VkImageViewCreateFlags flags = 0) {
    return this->createImageView(imageName, VK_IMAGE_VIEW_TYPE_CUBE, aspectMask,
                                 baseMipLevel, levelCount, baseArrayLayer, 6,
  ...
  uint32_t createImageView2D(uint32_t imageName, VkImageAspectFlags aspectMask,
    ...
    return this->createImageView(imageName, VK_IMAGE_VIEW_TYPE_2D, aspectMask,
                                 baseMipLevel, levelCount, baseArrayLayer, 1,
                                 componentMapping, flags);
  }

  void destroyImageView(uint32_t imageViewName) {
    assert(imageViewName < m_imageViews.size());
    assert(std::find(m_availableImageViewNames.begin(),
                     m_availableImageViewNames.end(),
                     imageViewName) == m_availableImageViewNames.end());
  ...
  // --- Image utilities ---
  void transitionImageLayout(uint32_t imageName, VkImageLayout oldLayout,
                             VkImageLayout newLayout) {
    auto &image = m_images[imageName];

    beginSingleTimeCommands();
    recordImageLayoutTransitionCommands(m_singleTimeCommandBuffer, image,
                                        image.format(), 0, image.levels(), 0,
                                        image.layers(), oldLayout, newLayout);
  ...
  void transferHostDataToImage(
      uint32_t imageName, VkDeviceSize sizeInBytes, const void *hostData,
      VkImageAspectFlags aspectMask, VkImageLayout currentLayout,
      VkImageLayout finalLayout = VK_IMAGE_LAYOUT_UNDEFINED) {
    if (!hostData)
      throw std::invalid_argument("hostData cannot be null");
    if (sizeInBytes == 0)
      throw std::invalid_argument("sizeInBytes cannot be 0");

    auto &image = m_images.at(imageName);

    transitionImageLayout(imageName, currentLayout,
    ...
    beginSingleTimeCommands();
    recordCopyBufferToImageCommands(
        m_singleTimeCommandBuffer, stagingBuffer, image, image.format(),
        aspectMask, image.extent().width, image.extent().height,
        image.extent().depth, image.levels(), image.layers());
    endSingleTimeCommands();
  ...
  void readImage(std::vector<char> &hostBuffer, uint32_t imageName,
                 VkImageAspectFlags aspectMask, VkImageLayout currentLayout) {
    auto &image = m_images.at(imageName);
    ...
    beginSingleTimeCommands();
    vkCmdCopyImageToBuffer(m_singleTimeCommandBuffer, image,
                           VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, stagingBuffer,
                           static_cast<uint32_t>(imageCopyRegions.size()),
  ...
  // --- Buffer related ---
  uint32_t createBuffer(VkDeviceSize sizeInBytes, VkBufferUsageFlags usage,
  ...
  void desctroyBuffer(uint32_t bufferName) {
    assert(std::find(m_availableBufferNames.begin(),
                     m_availableBufferNames.end(),
                     bufferName) == m_availableBufferNames.end());
    assert(bufferName < m_buffers.size());
    m_availableBufferNames.push_back(bufferName);
  }

  void transferHostDataToBuffer(uint32_t bufferName, VkDeviceSize sizeInBytes,
                                const void *hostData,
    ...
    beginSingleTimeCommands();
    recordCopyBufferToBufferCommands(m_singleTimeCommandBuffer, stagingBuffer,
                                     dstBuffer, sizeInBytes, 0, dstOffset);
    endSingleTimeCommands();
  }

  void *mapBuffer(uint32_t bufferName, VkDeviceSize offset = 0,
  ...
  uint32_t createSampler(
      ...
      VkSamplerAddressMode addressModeW, float minLod = 0.f, float maxLod = 0.f,
      float mipLodBias = 0.f, VkBool32 anisotropyEnable = VK_FALSE,
      float maxAnisotropy = 0.f, VkBool32 compareEnable = VK_FALSE,
      VkCompareOp compareOp = VK_COMPARE_OP_NEVER,
      VkBorderColor borderColor = VK_BORDER_COLOR_FLOAT_TRANSPARENT_BLACK,
      VkBool32 unnormailzedCoords = VK_FALSE, VkSamplerCreateFlags flags = 0) {
    uint32_t samplerName;
    ...
    m_samplers.at(samplerName)
        .init(magFilter, minFilter, mipmapMode, addressModeU, addressModeV,
              addressModeW, minLod, maxLod, mipLodBias, anisotropyEnable,
              maxAnisotropy, compareEnable, compareOp, borderColor,
              unnormailzedCoords, flags);
  ...
  void destroySampler(uint32_t samplerName) {
    ...
    assert(std::find(m_availableSamplerNames.begin(),
                     m_availableSamplerNames.end(),
                     samplerName) == m_availableSamplerNames.end());
  ...
  // --- Framebuffer related ---
  uint32_t createFramebuffer(uint32_t renderPassName,
                             const std::vector<uint32_t> &attachmentViewNames) {
    ...
    for (auto name : attachmentViewNames) {
      ...
      if (!first) {
        if (extent.width != width || extent.height != height ||
            extent.depth != 1 || view.layers() != layers) {
          throw std::runtime_error(
              "Image view cannot be used as framebuffer attachment");
        }
        first = false;
      }
    ...
    m_framebuffers[fbName].init(renderPass, attachmentViews, width, height,
                                layers);
    return fbName;
  }
  ...
  std::vector<uint32_t> createSwapChainFramebuffers(uint32_t renderPassName) {
    ...
    if (!m_swapChainFramebufferNames.empty()) {
      m_availableFramebufferNames.insert(m_availableFramebufferNames.end(),
                                         m_swapChainFramebufferNames.begin(),
                                         m_swapChainFramebufferNames.end());
    ...
    for (const auto &view : views) {
      ...
      m_framebuffers.at(fbName).init(renderPass, {view}, extent.width,
                                     extent.height, 1);
  ...
  void destroyFramebuffer(uint32_t framebufferName) {
    ...
    assert(std::find(m_availableFramebufferNames.begin(),
                     m_availableFramebufferNames.end(),
                     framebufferName) == m_availableFramebufferNames.end());

    if (std::find(m_swapChainFramebufferNames.begin(),
                  m_swapChainFramebufferNames.end(),
                  framebufferName) != m_swapChainFramebufferNames.end())
      return;
  ...
  void beginCreateDescriptorPool(uint32_t maxNumSets) {
    ...
    m_descriptorPools.emplace(std::piecewise_construct,
                              std::forward_as_tuple(m_curDescriptorPoolName),
  ...
  void resetDescriptorPool(uint32_t poolName,
    ...
    m_availableDescriptorSetNames.insert(m_availableDescriptorSetNames.end(),
                                         poolSets.begin(), poolSets.end());
    poolSets.clear();
  }
  // --- Descriptor pool related ---
  ...
  // --- Descriptor sets ---
  std::vector<uint32_t>
  allocateDescriptorSets(uint32_t descriptorPoolName,
                         const std::vector<uint32_t> &setLayoutNames) {
    ...
    if (vkAllocateDescriptorSets(m_device, &allocInfo, sets.data()) !=
        VK_SUCCESS) {
  ...
  void descriptorSetAddBufferDescriptor(
      uint32_t binding, VkDescriptorType type,
      const std::vector<DescriptorSetUpdateBufferInfo> &updateInfos,
  ...
  void descriptorSetAddImageDescriptor(
      uint32_t binding, VkDescriptorType type,
      const std::vector<DescriptorSetUpdateImageInfo> &updateInfos,
      uint32_t baseArrayElement = 0) {
    assert(!updateInfos.empty());
    ...
    for (const auto &updateInfo : updateInfos) {
      ...
      info.sampler =
          updateInfo.samplerName == std::numeric_limits<uint32_t>::max()
              ? VK_NULL_HANDLE
              : VkSampler(m_samplers.at(updateInfo.samplerName));
  ...
  void endUpdateDescriptorSet() {
    vkUpdateDescriptorSets(
        m_device,
        static_cast<uint32_t>(m_curDescriptorSetInfo.writeInfos.size()),
  ...
  uint32_t createCommandPool(VkQueueFlagBits submitQueueType,
    ...
    default:
      throw std::invalid_argument(
          "unsupported queue type specified during command pool creation");
    }

    uint32_t newPoolName;
    newPoolName = static_cast<uint32_t>(m_commandPools.size());
    m_commandPools.emplace(
        std::piecewise_construct, std::forward_as_tuple(newPoolName),
    ...
    if (vkCreateCommandPool(m_device, &poolInfo, nullptr, pool.replace()) !=
        VK_SUCCESS) {
  ...
  void resetCommandPool(uint32_t commandPoolName,
                        VkCommandPoolResetFlags flags = 0) {
    std::lock_guard<std::shared_mutex> guard(
        g_commandBufferMutex); // no command buffer is in-use
    ...
    if (flags & VK_COMMAND_POOL_RESET_RELEASE_RESOURCES_BIT) {
      ...
      m_commandBufferAvaiableNames.insert(m_commandBufferAvaiableNames.end(),
                                          cbNames.begin(), cbNames.end());
      cbNames.clear();
    }
  }
  // --- Command pool related ---
  ...
  void endSingleTimeCommands() {
    ...
    vkFreeCommandBuffers(m_device,
                         m_commandPools[m_singleSubmitCommandPoolName], 1,
                         &m_singleTimeCommandBuffer);

    m_singleTimeCommandBuffer = VK_NULL_HANDLE;
  }

  std::vector<uint32_t> allocateCommandBuffers(
      uint32_t poolName, uint32_t count,
    ...
    if (vkAllocateCommandBuffers(m_device, &allocInfo, cbs.data()) !=
        VK_SUCCESS) {
  ...
  void beginCommandBuffer(uint32_t commandBufferName,
  ...
  void cmdBindVertexBuffers(uint32_t cmdBufferName,
                            const std::vector<uint32_t> &bufferNames,
    ...
    vkCmdBindVertexBuffers(cmdBuffer, firstBinding, numVertBuffers,
                           vertBuffers.data(), offsets.data());
  }

  void cmdBindIndexBuffer(uint32_t cmdBufferName, uint32_t indexBufferName,
                          VkIndexType type, VkDeviceSize offset = 0) const {
    const auto &cmdBuffer = m_commandBuffers.at(cmdBufferName);
  ...
  void cmdBeginRenderPass(
      uint32_t cmdBufferName, uint32_t renderPassName, uint32_t frameBufferName,
      const std::vector<VkClearValue> &clearValues, VkRect2D renderArea = {},
    ...
    renderArea.extent.width = renderArea.extent.width == 0
                                  ? framebuffer.width()
                                  : renderArea.extent.width;
    renderArea.extent.height = renderArea.extent.height == 0
                                   ? framebuffer.height()
  ...
  void cmdBindPipeline(uint32_t cmdBufferName,
                       VkPipelineBindPoint pipelineBindPoint,
                       uint32_t pipelineName) const {
    const auto &cmdBuffer = m_commandBuffers.at(cmdBufferName);
  ...
  void cmdBindDescriptorSets(
      uint32_t cmdBufferName, VkPipelineBindPoint pipelineBindPoint,
      uint32_t pipelineLayoutName,
      const std::vector<uint32_t> &descriptorSetNames, uint32_t firstSet = 0,
    ...
    vkCmdBindDescriptorSets(cmdBuffer, pipelineBindPoint, pipelineLayout,
                            firstSet, numSets, sets.data(), numDynamicOffsets,
                            numDynamicOffsets == 0 ? nullptr
                                                   : dynamicOffsets.data());
  }

  void cmdSetViewport(uint32_t cmdBufferName, uint32_t framebufferName,
                      float topLeftU = 0.f, float topLeftV = 0.f,
  ...
  void cmdSetViewport(uint32_t cmdBufferName, float x, float y, float width,
                      float height, float minDepth = 0.f,
                      float maxDepth = 1.f) const {
    const auto &cmdBuffer = m_commandBuffers.at(cmdBufferName);
  ...
  void cmdSetScissor(uint32_t cmdBufferName, uint32_t framebufferName,
                     float topLeftU = 0.f, float topLeftV = 0.f,
                     float width = 1.f, float height = 1.f) const {
    const auto &cmdBuffer = m_commandBuffers.at(cmdBufferName);
    const auto &framebuffer = m_framebuffers.at(framebufferName);
  ...
  void cmdSetScissor(uint32_t cmdBufferName, int32_t x, int32_t y,
  ...
  void cmdDrawIndexed(uint32_t cmdBufferName, uint32_t indexCount,
  ...
  void cmdDraw(uint32_t cmdBufferName, uint32_t vertexCount,
  ...
  void cmdPushConstants(uint32_t cmdBufferName, uint32_t pipelineLayoutName,
                        VkShaderStageFlags shaderStages, uint32_t offset,
                        uint32_t sizeInBytes, const void *pValues) const {
    const auto &cmdBuffer = m_commandBuffers.at(cmdBufferName);
    const auto &pipelineLayout = m_pipelineLayouts.at(pipelineLayoutName);

    vkCmdPushConstants(cmdBuffer, pipelineLayout, shaderStages, offset,
                       sizeInBytes, pValues);
  }

  void cmdDispatch(uint32_t cmdBufferName, uint32_t numBlocksX,
                   uint32_t numBlocksY, uint32_t numBlocksZ) {
    const auto &cmdBuffer = m_commandBuffers.at(cmdBufferName);
  ...
  void queueSubmitNewSubmit(
      const std::vector<uint32_t> &cmdBufferNames,
  ...
  void endQueueSubmit(uint32_t fenceName = std::numeric_limits<uint32_t>::max(),
                      bool waitForFence = true) {
    uint32_t numSubmits = static_cast<uint32_t>(m_curQueueSubmitInfos.size());
    assert(numSubmits > 0);
    std::vector<VkSubmitInfo> infos(numSubmits, VkSubmitInfo());
    ...
    for (uint32_t i = 0; i < numSubmits; ++i) {
      ...
      info.waitSemaphoreCount =
          static_cast<uint32_t>(src.waitSemaphores.size());
      info.pWaitSemaphores = src.waitSemaphores.data();
      info.pWaitDstStageMask = src.waitStages.data();
      info.signalSemaphoreCount =
          static_cast<uint32_t>(src.signalSemaphores.size());
      info.pSignalSemaphores = src.signalSemaphores.data();
    }

    assert(m_curSubmitQueue);
    VkFence fence = fenceName == std::numeric_limits<uint32_t>::max()
                        ? VK_NULL_HANDLE
                        : VkFence(m_fences[fenceName]);
    if (vkQueueSubmit(m_curSubmitQueue, numSubmits, infos.data(), fence) !=
        VK_SUCCESS) {
      throw std::runtime_error("failed to submit");
    }

    if (fence && waitForFence) {
      vkWaitForFences(m_device, 1, &fence, VK_TRUE,
  ...
  uint32_t createSemaphore(VkSemaphoreCreateFlags flags = 0) {
    ...
    if (vkCreateSemaphore(m_device, &info, nullptr,
                          m_semaphores[semaphoreName].replace()) !=
        VK_SUCCESS) {
  ...
  uint32_t createFence(VkFenceCreateFlags flags = 0) {
    ...
    if (vkCreateFence(m_device, &info, nullptr,
                      m_fences[fenceName].replace()) != VK_SUCCESS) {
  ...
  void waitForFences(const std::vector<uint32_t> &fenceNames,
    ...
    if (vkWaitForFences(m_device, fenceCount, fences.data(), waitAll,
                        timeout) != VK_SUCCESS) {
  ...
  VkExtent2D getSwapChainExtent() const { return m_swapChain.extent(); }

  VkFormat getSwapChainImageFormat() const { return m_swapChain.format(); }
  ...
  VkResult swapChainNextImageIndex(
      uint32_t *pIdx, uint32_t signalSemaphoreName, uint32_t waitFenceName,
      uint64_t timeout = std::numeric_limits<uint64_t>::max()) {
    VkSemaphore semaphore =
        signalSemaphoreName == std::numeric_limits<uint32_t>::max()
            ? VK_NULL_HANDLE
            : VkSemaphore(m_semaphores[signalSemaphoreName]);
    VkFence fence = waitFenceName == std::numeric_limits<uint32_t>::max()
                        ? VK_NULL_HANDLE
                        : VkFence(m_fences[waitFenceName]);

    return vkAcquireNextImageKHR(m_device, m_swapChain, timeout, semaphore,
                                 fence, pIdx);
  }

  VkResult queuePresent(const std::vector<uint32_t> &waitSemaphoreNames,
                        uint32_t imageIdx) {
  ...
  void windowPollEvents() const { glfwPollEvents(); }
  ...
  VkFormat
  chooseSupportedFormatFromCandidates(const std::vector<VkFormat> &candidates,
...
protected:
  void createPipelineCache() {
    ...
    if (vkCreatePipelineCache(m_device, &pipelineCacheCreateInfo, nullptr,
                              m_pipelineCache.replace()) != VK_SUCCESS) {
  ...
  std::unordered_map<uint32_t, VDeleter<VkDescriptorSetLayout>>
  ...
  std::unordered_map<uint32_t, std::vector<uint32_t>>
      m_commandBufferTable; // command buffers of each pool
  ...
  std::unordered_map<uint32_t, std::vector<uint32_t>>
      m_poolSetTable; // sets from each pool
