...
public:
  VInstance(bool enableValidationLayers,
      ...
      : m_enableValidationLayers(enableValidationLayers),
        m_layerNames(layerNames), m_requiredExtensions(extensionNames) {
  ...
  operator const VDeleter<VkInstance> &() const { return m_instance; }

protected:
  void createInstance() {
    if (m_enableValidationLayers &&
        !checkValidationLayerSupport(m_layerNames)) {
  ...
  void setupDebugCallback() {
    ...
    if (createDebugReportCallbackEXT(m_instance, &createInfo, nullptr,
                                     m_debugReportCB.replace()) != VK_SUCCESS) {
  ...
  VDeleter<VkDebugReportCallbackEXT> m_debugReportCB{
      m_instance, destroyDebugReportCallbackEXT};
};
}