...
template <size_t maxSizeInBytes> class UniformBlob {
public:
  UniformBlob(
      VkDeviceSize alignmentInBytes = std::numeric_limits<VkDeviceSize>::max())
      : memory(new char[maxSizeInBytes]), minAlignment(alignmentInBytes) {}

  virtual ~UniformBlob() { delete[] memory; }
  ...
  void *alloc(size_t size) {
    ...
    if (size == 0)
      throw std::runtime_error("UniformBlob::alloc - size can't be zero.");

    size_t actualSize = (size / minAlignment) * minAlignment;
    actualSize = actualSize < size ? (actualSize + minAlignment) : actualSize;

    if (nextStartingByte + actualSize > maxSizeInBytes)
      throw std::runtime_error("UniformBlob::alloc - out of memory.");
  ...
  size_t size() const { return currentSizeInBytes; }

  const char *operator&() const { return memory; }

  // @ptr should be a pointer to somewhere in @memory
  size_t offsetOf(const char *ptr) const { return ptr - memory; }
...
public:
  Timer(uint32_t ac = 100)
      : tStart(std::chrono::high_resolution_clock::now()), tEnd(tStart),
        averageCount{ac}, timeIntervals(ac, 0.f) {}
  ...
  float getAverageTime() const { return total / averageCount; }
...
size_t compute2DImageSizeInBytes(uint32_t width, uint32_t height,
...
// Address computation: layerIdx * layerSize + faceIdx * faceSize + levelSize(0)
// + ... + levelSize(levelIdx - 1)
// layerCount is always 1 for 2D image (you need a texture2d_array to use
// layers)
// faceCount is always 1 for 2D image
void saveImage2D(const std::string &fileName, uint32_t width, uint32_t height,
                 uint32_t bytesPerPixel, uint32_t mipLevels, gli::format format,
  ...
  size_t sizeInBytes =
      compute2DImageSizeInBytes(width, height, bytesPerPixel, mipLevels, 1);
...
void saveImageCube(const std::string &fileName, uint32_t width, uint32_t height,
                   uint32_t bytesPerPixel, uint32_t mipLevels,
                   gli::format format, const void *pixelData) {
  assert(width > 0 && height > 0);
  assert(mipLevels > 0);
  assert(pixelData);
  ...
  size_t sizeInBytes =
      compute2DImageSizeInBytes(width, height, bytesPerPixel, mipLevels, 6);
  memcpy(image.data(), pixelData, sizeInBytes);
...
void createShaderModule(VDeleter<VkShaderModule> &shaderModule, VkDevice device,
...
void createImageView(
    VDeleter<VkImageView> &imageView, VkDevice device, VkImage image,
...
// --- Image creation ---
void createImage(VDeleter<VkImage> &image,
...
void recordImageLayoutTransitionCommands(
    VkCommandBuffer commandBuffer, VkImage image, VkFormat format,
    uint32_t baseLevel, uint32_t mipLevelCount, uint32_t baseLayer,
  ...
  // barrier.srcAccessMask - operations involving the resource that must happen
  // before the barrier
  // barrier.dstAccessMask - operations involving the resource that must wait on
  // the barrier
...
// --- Buffer creation ---
void createBuffer(VDeleter<VkBuffer> &buffer,
  ...
  bufferInfo.pQueueFamilyIndices =
      queueFamilyIndices.size() == 0 ? nullptr : queueFamilyIndices.data();
...
void recordCopyBufferToImageCommands(
    VkCommandBuffer commandBuffer, VkBuffer srcBuffer, VkImage dstImage,
    VkFormat format, VkImageAspectFlags aspectMask, uint32_t width,
    uint32_t height, uint32_t depth = 1, uint32_t levelCount = 1,
    uint32_t layerCount = 1) {
