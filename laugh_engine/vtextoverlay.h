...
public:
  ...
  VTextOverlay(rj::VManager *manager) : pManager(manager) {}
  ...
  void beginTextUpdate() {
    mapped = reinterpret_cast<glm::vec4 *>(
        pManager->mapBuffer(fontQuadVertexBuffer.buffer));
    numLetters = 0;
  }

  void addText(std::string text, float x, float y,
               TextAlign align = alignLeft) {
    ...
    for (auto letter : text) {
      stb_fontchar *charData =
          &fontDescriptors[(uint32_t)letter - STB_FIRST_CHAR];
  ...
  void updateCommandBuffers(uint32_t imageIdx) {
    ...
    pManager->cmdBeginRenderPass(commandBuffers[imageIdx], renderPass,
                                 framebuffers[imageIdx], {});

    pManager->cmdBindPipeline(commandBuffers[imageIdx],
    ...
    pManager->cmdBindDescriptorSets(commandBuffers[imageIdx],
                                    VK_PIPELINE_BIND_POINT_GRAPHICS,
                                    pipelineLayout, {descriptorSet});

    pManager->cmdBindVertexBuffers(commandBuffers[imageIdx],
                                   {fontQuadVertexBuffer.buffer}, {0});
  ...
  void submit(uint32_t bufferindex, const std::vector<uint32_t> &waitSemaphores,
              const std::vector<VkPipelineStageFlags> &waitStages,
              const std::vector<uint32_t> &signalSemaphores) const {
    pManager->beginQueueSubmit(VK_QUEUE_GRAPHICS_BIT);
    pManager->queueSubmitNewSubmit({commandBuffers[bufferindex]},
...
private:
  ...
  stb_fontchar fontDescriptors[STB_NUM_CHARS]; // meta info like x/y offsets to
                                               // upper left corner of a
                                               // character window. s/t texture
                                               // coordinates
  ...
  void createCommandPools() {
    ...
    commandPool = pManager->createCommandPool(
        VK_QUEUE_GRAPHICS_BIT, VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT);
  ...
  void createFontTexture() {
    unsigned char fontTexture[STB_FONT_HEIGHT][STB_FONT_WIDTH]; // a texture
                                                                // atlas
                                                                // containing
                                                                // all the
                                                                // characters
    ...
    fontDeviceTexture.image = pManager->createImage2D(
        STB_FONT_WIDTH, STB_FONT_HEIGHT, fontDeviceTextureFormat,
        VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

    pManager->transferHostDataToImage(
        fontDeviceTexture.image,
        STB_FONT_HEIGHT * STB_FONT_WIDTH * sizeof(unsigned char), fontTexture,
        VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_PREINITIALIZED,
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

    // create image view
    fontDeviceTexture.imageViews.resize(1);
    fontDeviceTexture.imageViews[0] = pManager->createImageView2D(
        fontDeviceTexture.image, VK_IMAGE_ASPECT_COLOR_BIT);

    // create sampler
    fontDeviceTexture.samplers.resize(1);
    fontDeviceTexture.samplers[0] = pManager->createSampler(
  ...
  void createVertexBuffer() {
    ...
    fontQuadVertexBuffer.size =
        MAX_CHAR_COUNT * sizeof(glm::vec4); // (x, y, s, t)

    fontQuadVertexBuffer.buffer = pManager->createBuffer(
        fontQuadVertexBuffer.size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
  ...
  void createDescriptorPoolAndSetLayouts() {
    ...
    pManager->setLayoutAddBinding(0, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                                  VK_SHADER_STAGE_FRAGMENT_BIT);
    descriptorSetLayout = pManager->endCreateDescriptorSetLayout();

    // descriptor pool
    pManager->beginCreateDescriptorPool(1);
    pManager->descriptorPoolAddDescriptors(
  ...
  void createDescriptorSets() {
    auto sets =
        pManager->allocateDescriptorSets(descriptorPool, {descriptorSetLayout});
    ...
    pManager->descriptorSetAddImageDescriptor(
        0, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, imageInfos);
  ...
  void createRenderPasses() {
    ...
    pManager->renderPassAddAttachment(
        swapChainFormat, VK_IMAGE_LAYOUT_UNDEFINED,
        VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VK_SAMPLE_COUNT_1_BIT,
        VK_ATTACHMENT_LOAD_OP_LOAD);

    pManager->beginDescribeSubpass();
    pManager->subpassAddColorAttachmentReference(
        0, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
    pManager->endDescribeSubpass();

    pManager->renderPassAddSubpassDependency(
  ...
  void createPipelines() {
    ...
    pManager->graphicsPipelineAddShaderStage(VK_SHADER_STAGE_VERTEX_BIT,
                                             vsFileName);
    pManager->graphicsPipelineAddShaderStage(VK_SHADER_STAGE_FRAGMENT_BIT,
                                             fsFileName);

    auto bindingDesc = TextQuadVertex::getBindingDescription();
    pManager->graphicsPipelineAddBindingDescription(
        bindingDesc.binding, bindingDesc.stride, bindingDesc.inputRate);
    auto attrDescs = TextQuadVertex::getAttributeDescriptions();
    for (const auto &attrDesc : attrDescs) {
      pManager->graphicsPipelineAddAttributeDescription(
          attrDesc.location, attrDesc.binding, attrDesc.format,
          attrDesc.offset);
    }

    pManager->graphicsPipelineConfigureInputAssembly(
        VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP);

    pManager->graphicsPipeLineAddColorBlendAttachment(
        VK_TRUE, VK_BLEND_FACTOR_ONE, VK_BLEND_FACTOR_ONE, VK_BLEND_OP_ADD);

    pManager->graphicsPipelineConfigureDepthState(VK_FALSE, VK_FALSE,
