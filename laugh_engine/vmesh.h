...
struct BBox {
  ...
  BBox(const glm::vec3 &_min, const glm::vec3 &_max) : min(_min), max(_max) {}
};
...
void loadMeshIntoHostBuffers(const std::string &modelFileName,
                             ...
                             glm::vec3 *minPos = nullptr,
                             glm::vec3 *maxPos = nullptr) {
  if (minPos)
...
void loadTexture2DFromBinaryData(ImageWrapper *pTexRet, VManager *pManager,
                                 const void *pixels, uint32_t width,
                                 uint32_t height, gli::format gliformat,
  ...
  size_t sizeInBytes = compute2DImageSizeInBytes(
      width, height, formatInfo.blockSize, mipLevels, 1);
  ...
  pTexRet->image = pManager->createImage2D(
      width, height, format,
      VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, mipLevels);

  pManager->transferHostDataToImage(
      pTexRet->image, sizeInBytes, textureSrc.data(), VK_IMAGE_ASPECT_COLOR_BIT,
      VK_IMAGE_LAYOUT_PREINITIALIZED, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

  pTexRet->imageViews.push_back(pManager->createImageView2D(
      pTexRet->image, VK_IMAGE_ASPECT_COLOR_BIT, 0, mipLevels));

  if (createSampler) {
    pTexRet->samplers.resize(1);
    pTexRet->samplers[0] = pManager->createSampler(
        VK_FILTER_LINEAR, VK_FILTER_LINEAR, VK_SAMPLER_MIPMAP_MODE_LINEAR,
        VK_SAMPLER_ADDRESS_MODE_REPEAT, VK_SAMPLER_ADDRESS_MODE_REPEAT,
        VK_SAMPLER_ADDRESS_MODE_REPEAT, 0.f, float(mipLevels - 1), 0.f, VK_TRUE,
        16.f);
  }
}

void loadTexture2D(ImageWrapper *pTexRet, VManager *pManager,
                   const std::string &fn, bool createSampler = true) {
...
void loadCubemap(ImageWrapper *pTexRet, VManager *pManager,
                 const std::string &fn, bool createSampler = true) {
  std::string ext = getFileExtension(fn);
  if (ext != "ktx" && ext != "dds") {
    throw std::runtime_error("texture type ." + ext + " is not supported.");
  ...
  pTexRet->image = pManager->createImageCube(
      width, height, format,
      VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, mipLevels);

  pManager->transferHostDataToImage(
      pTexRet->image, sizeInBytes, texCube.data(), VK_IMAGE_ASPECT_COLOR_BIT,
      VK_IMAGE_LAYOUT_PREINITIALIZED, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

  pTexRet->imageViews.push_back(pManager->createImageViewCube(
      pTexRet->image, VK_IMAGE_ASPECT_COLOR_BIT, 0, mipLevels));
  ...
  if (createSampler) {
    ...
    pTexRet->samplers[0] = pManager->createSampler(
        ...
        VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE,
        VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE, 0.f, float(mipLevels - 1), 0.f,
        VK_TRUE, 16.f);
  }
}
}
...
public:
  ...
  glm::quat worldRotation{
      glm::vec3(0.f, 0.f, 0.f)}; // Euler angles to quaternion
  float scale = 1.f;
  BBox bounds;
  ...
  static void loadFromGLTF(std::vector<VMesh> &retMeshes,
    ...
    if (version == "1.0") {
      ...
      // Meshes
      std::unordered_map<std::string, std::vector<const tinygltf::Primitive *>>
          mat2meshes;
      ...
      for (const auto &matMeshes : mat2meshes) {
        ...
        {
          const auto &texName =
              material.values.at("baseColorTexture").string_value;
          ...
          loadTexture2DFromBinaryData(
              &retMesh.albedoMap, pManager, image.image.data(), image.width,
              image.height, gliFormat, image.levelCount);
        }
        {
          const auto &texName =
              material.values.at("normalTexture").string_value;
          const auto &tex = textures.at(texName);
          const auto &image = images.at(tex.source);

          auto gliFormat = chooseFormat(tex.type, image.component);
          loadTexture2DFromBinaryData(
              &retMesh.normalMap, pManager, image.image.data(), image.width,
              image.height, gliFormat, image.levelCount);
        }
        {
          const auto &texName =
              material.values.at("roughnessTexture").string_value;
          const auto &tex = textures.at(texName);
          const auto &image = images.at(tex.source);

          auto gliFormat = chooseFormat(tex.type, image.component);
          loadTexture2DFromBinaryData(
              &retMesh.roughnessMap, pManager, image.image.data(), image.width,
              image.height, gliFormat, image.levelCount);
        }
        {
          const auto &texName =
              material.values.at("metallicTexture").string_value;
          const auto &tex = textures.at(texName);
          const auto &image = images.at(tex.source);

          auto gliFormat = chooseFormat(tex.type, image.component);
          loadTexture2DFromBinaryData(
              &retMesh.metalnessMap, pManager, image.image.data(), image.width,
              image.height, gliFormat, image.levelCount);
        }
        if (material.values.find("aoTexture") != material.values.end()) {
          ...
          auto gliFormat = chooseFormat(tex.type, image.component);
          loadTexture2DFromBinaryData(
              &retMesh.aoMap, pManager, image.image.data(), image.width,
              image.height, gliFormat, image.levelCount);
        }
        if (material.values.find("emissiveTexture") != material.values.end()) {
          const auto &texName =
              material.values.at("emissiveTexture").string_value;
          const auto &tex = textures.at(texName);
          const auto &image = images.at(tex.source);

          auto gliFormat = chooseFormat(tex.type, image.component);
          loadTexture2DFromBinaryData(
              &retMesh.emissiveMap, pManager, image.image.data(), image.width,
              image.height, gliFormat, image.levelCount);
        }
        ...
        for (const auto &pMeshPrim : matMeshes.second) {
          ...
          const auto &posAccessor =
              accessors.at(mesh.attributes.at("POSITION"));
          const auto &nrmAccessor = accessors.at(mesh.attributes.at("NORMAL"));
          const auto &uvAccessor =
              accessors.at(mesh.attributes.at("TEXCOORD_0"));
          ...
          // Postions
          {
            ...
            for (uint32_t i = 0; i < numVertices; ++i) {
              ...
              hostVertices[vertOffset + i].pos =
                  glm::vec3(pos[0], pos[1], pos[2]);

              retMesh.bounds.max = glm::max(retMesh.bounds.max,
                                            hostVertices[vertOffset + i].pos);
              retMesh.bounds.min = glm::min(retMesh.bounds.max,
                                            hostVertices[vertOffset + i].pos);
            }
          }
          // Normal
          {
            ...
            for (uint32_t i = 0; i < numVertices; ++i) {
              const float *nrm =
                  reinterpret_cast<const float *>(&data[stride * i]);
              hostVertices[vertOffset + i].normal =
                  glm::vec3(nrm[0], nrm[1], nrm[2]);
            }
          }
          // Texture coordinates
          {
            ...
            for (uint32_t i = 0; i < numVertices; ++i) {
              const float *uv =
                  reinterpret_cast<const float *>(&data[stride * i]);
              hostVertices[vertOffset + i].texCoord =
                  glm::vec2(uv[0], 1.f - uv[1]);
            }
          }
          // Indices
          {
            ...
            const uint16_t *data =
                reinterpret_cast<const uint16_t *>(&buffer.data[offset]);

            for (uint32_t i = 0; i < numIndices; ++i) {
              hostIndices[indexOffset + i] =
                  vertOffset + static_cast<uint32_t>(data[i]);
            }
          }
        ...
        retMesh.vertexBuffer.size =
            sizeof(hostVertices[0]) * hostVertices.size();
        retMesh.vertexBuffer.buffer = pManager->createBuffer(
            retMesh.vertexBuffer.size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT |
                                           VK_BUFFER_USAGE_TRANSFER_DST_BIT,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

        pManager->transferHostDataToBuffer(retMesh.vertexBuffer.buffer,
                                           retMesh.vertexBuffer.size,
                                           hostVertices.data());
        ...
        retMesh.indexBuffer.buffer = pManager->createBuffer(
            retMesh.indexBuffer.size,
            VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

        pManager->transferHostDataToBuffer(retMesh.indexBuffer.buffer,
                                           retMesh.indexBuffer.size,
                                           hostIndices.data());
      }
    } else {
      ...
      for (const auto &mesh : scene.meshes) {
        ...
        // Textures
        loadTexture2DFromBinaryData(&retMesh.albedoMap, pManager,
                                    mesh.albedoMap.pixels.data(),
                                    mesh.albedoMap.width, mesh.albedoMap.height,
                                    gliFormat, mesh.albedoMap.levelCount);

        loadTexture2DFromBinaryData(&retMesh.normalMap, pManager,
                                    mesh.normalMap.pixels.data(),
                                    mesh.normalMap.width, mesh.normalMap.height,
                                    gliFormat, mesh.normalMap.levelCount);

        loadTexture2DFromBinaryData(
            &retMesh.roughnessMap, pManager, mesh.roughnessMap.pixels.data(),
            mesh.roughnessMap.width, mesh.roughnessMap.height, gliFormat,
            mesh.roughnessMap.levelCount);

        loadTexture2DFromBinaryData(
            &retMesh.metalnessMap, pManager, mesh.metallicMap.pixels.data(),
            mesh.metallicMap.width, mesh.metallicMap.height, gliFormat,
            mesh.metallicMap.levelCount);

        if (!mesh.aoMap.pixels.empty()) {
          loadTexture2DFromBinaryData(&retMesh.aoMap, pManager,
                                      mesh.aoMap.pixels.data(),
                                      mesh.aoMap.width, mesh.aoMap.height,
                                      gliFormat, mesh.aoMap.levelCount);
        }

        if (!mesh.emissiveMap.pixels.empty()) {
          loadTexture2DFromBinaryData(
              &retMesh.emissiveMap, pManager, mesh.emissiveMap.pixels.data(),
              mesh.emissiveMap.width, mesh.emissiveMap.height, gliFormat,
              mesh.emissiveMap.levelCount);
        }
        ...
        for (int i = 0; i < vertCount; ++i) {
          ...
          vert.pos = glm::vec3(mesh.positions[3 * i], mesh.positions[3 * i + 1],
                               mesh.positions[3 * i + 2]);
          vert.normal = glm::vec3(mesh.normals[3 * i], mesh.normals[3 * i + 1],
                                  mesh.normals[3 * i + 2]);
          vert.texCoord =
              glm::vec2(mesh.texCoords[i << 1], mesh.texCoords[(i << 1) + 1]);
  ...
  void load(const std::string &modelFileName,
    ...
    loadMeshIntoHostBuffers(modelFileName, hostVerts, hostIndices, &minPos,
                            &maxPos);
    bounds.min = minPos;
    bounds.max = maxPos;
    ...
    vertexBuffer.buffer = pVulkanManager->createBuffer(
        vertexBuffer.size,
        VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

    pVulkanManager->transferHostDataToBuffer(
        vertexBuffer.buffer, vertexBuffer.size, hostVerts.data());

    // create index buffer
    indexBuffer = {};
    indexBuffer.size = sizeof(hostIndices[0]) * hostIndices.size();
    indexBuffer.buffer = pVulkanManager->createBuffer(
        indexBuffer.size,
        VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

    pVulkanManager->transferHostDataToBuffer(
        indexBuffer.buffer, indexBuffer.size, hostIndices.data());
  }

  virtual void updateHostUniformBuffer() {
    assert(uPerModelInfo);
    if (!uniformDataChanged)
      return;
    uPerModelInfo->M =
        glm::translate(glm::mat4_cast(worldRotation) *
...
public:
  ...
  Skybox(rj::VManager *pManager) : VMesh{pManager} {
    materialType = MATERIAL_TYPE_HDR_PROBE;
  }

  void load(const std::string &modelFileName,
            const std::string &radianceMapName, const std::string &specMapName,
    ...
    if (specMapName != "") {
      ...
      uint32_t mipLevels =
          static_cast<uint32_t>(floor(log2f(SPEC_IRRADIANCE_MAP_SIZE) + 0.5f)) +
          1;

      specularIrradianceMap.format = VK_FORMAT_R32G32B32A32_SFLOAT;
      specularIrradianceMap.width = specularIrradianceMap.height =
          SPEC_IRRADIANCE_MAP_SIZE;
      specularIrradianceMap.depth = 1;
      specularIrradianceMap.mipLevelCount = mipLevels;
      specularIrradianceMap.layerCount = 6;

      specularIrradianceMap.image = pVulkanManager->createImageCube(
          specularIrradianceMap.width, specularIrradianceMap.height,
          specularIrradianceMap.format,
      ...
      specularIrradianceMap.imageViews[0] = pVulkanManager->createImageViewCube(
          specularIrradianceMap.image, VK_IMAGE_ASPECT_COLOR_BIT, 0, mipLevels);

      // Used for rendering
      for (uint32_t level = 0; level < mipLevels; ++level) {
        specularIrradianceMap.imageViews[level + 1] =
            pVulkanManager->createImageViewCube(
                specularIrradianceMap.image, VK_IMAGE_ASPECT_COLOR_BIT, level);
      }

      specularIrradianceMap.samplers.resize(1);
      specularIrradianceMap.samplers[0] = pVulkanManager->createSampler(
          VK_FILTER_LINEAR, VK_FILTER_LINEAR, VK_SAMPLER_MIPMAP_MODE_LINEAR,
          VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE,
          VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE,
          VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE, 0.f,
          static_cast<float>(mipLevels - 1), 0.f, VK_TRUE, 16.f);
    ...
    if (diffuseSHName != "") {
      ...
      computeSHCoefficients(radianceMapName,
                            rj::helper_functions::getBaseDir(radianceMapName) +
                                "/Diffuse_SH.bin");
    }
...
private:
  void computeSHCoefficients(const std::string &radianceMapName,
                             const std::string &saveFileName = "") {
    gli::texture_cube rm(gli::load(radianceMapName));
    if (rm.empty())
      throw std::runtime_error("Failed to load: " + radianceMapName);
    ...
    for (uint32_t faceIdx = 0; faceIdx < 6; ++faceIdx) {
      const glm::vec4 *rgba =
          reinterpret_cast<const glm::vec4 *>(rm.data(0, faceIdx, 0));
      ...
      for (uint32_t py = 0; py < height; ++py) {
        for (uint32_t px = 0; px < width; ++px) {
          ...
          wi = glm::normalize(wi);
          float dw = pixelArea * glm::dot(faceNrm, -wi) /
                     dist2; // differential solid angle
          glm::vec3 L = glm::vec3(rgba[py * width + px]);

          diffuseSHCoefficients[0] += L * 0.282095f * dw; // l = m = 0
          diffuseSHCoefficients[1] +=
              L * 0.488603f * wi.y * dw; // l = 1, m = -1
          diffuseSHCoefficients[2] += L * 0.488603f * wi.z * dw; // l = 1, m = 0
          diffuseSHCoefficients[3] += L * 0.488603f * wi.x * dw; // l = 1, m = 1
          diffuseSHCoefficients[4] +=
              L * 1.092548f * wi.x * wi.y * dw; // l = 2, m = -2
          diffuseSHCoefficients[5] +=
              L * 1.092548f * wi.y * wi.z * dw; // l = 2, m = -1
          diffuseSHCoefficients[6] +=
              L * 0.315392f * (3.f * wi.z * wi.z - 1.f) * dw; // l = 2, m = 0
          diffuseSHCoefficients[7] +=
              L * 1.092548f * wi.x * wi.z * dw; // l = 2, m = 1
          diffuseSHCoefficients[8] +=
              L * 0.546274f * (wi.x * wi.x - wi.y * wi.y) * dw; // l = 2, m = 2
        }
      }
    }
    ...
    if (saveFileName != "") {
      ...
      if (fs.is_open()) {
        fs.write(reinterpret_cast<const char *>(diffuseSHCoefficients),
                 sizeof(diffuseSHCoefficients));
        fs.close();
      } else {
  ...
  glm::vec3 getWorldDir(uint32_t faceIdx, uint32_t px, uint32_t py,
                        uint32_t width, uint32_t height) const {
    glm::vec2 pixelSize =
        1.f / glm::vec2(static_cast<float>(width), static_cast<float>(height));
    glm::vec2 uv = glm::vec2(static_cast<float>(px) + 0.5f,
    ...
    switch (faceIdx) {
    case 0: // +x
      return glm::vec3(0.5f, uv.y,
                       -uv.x); // DDS uses left-handed system. Need to flip z
    case 1:                    // -x
