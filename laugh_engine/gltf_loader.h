...
template <typename T> class GLTFBufferIterator {
public:
...
public:
  void load(GLTFScene *scene, const std::string &fn) const {
    ...
    if (ext != "gltf")
      throw std::runtime_error("Not gltf file");
    ...
    // Parse accessors
    if (!rootNode.contains("accessors") ||
        !rootNode.get("accessors").is<picojson::array>())
      throw std::runtime_error("Invalid accessors");
    std::vector<GLTFAccessor> accessors;
    parseAccessors(accessors, rootNode.get("accessors").get<picojson::array>());

    // Parse bufferViews
    if (!rootNode.contains("bufferViews") ||
        !rootNode.get("bufferViews").is<picojson::array>())
      throw std::runtime_error("Invalid bufferViews");
    std::vector<GLTFBufferView> bufferViews;
    parseBufferViews(bufferViews,
                     rootNode.get("bufferViews").get<picojson::array>());

    // Parse buffers
    if (!rootNode.contains("buffers") ||
        !rootNode.get("buffers").is<picojson::array>())
    ...
    // Parse images
    if (!rootNode.contains("images") ||
        !rootNode.get("images").is<picojson::array>())
      throw std::runtime_error("Invalid images");
    std::vector<GLTFImage> images;
    parseImages(images, rootNode.get("images").get<picojson::array>(), baseDir);

    // Parse textures
    if (!rootNode.contains("textures") ||
        !rootNode.get("textures").is<picojson::array>())
      throw std::runtime_error("Invalid textures");
    std::vector<GLTFTexture> textures;
    parseTextures(textures, rootNode.get("textures").get<picojson::array>());

    // Parse materials
    if (!rootNode.contains("materials") ||
        !rootNode.get("materials").is<picojson::array>())
      throw std::runtime_error("Invalid materials");
    std::vector<GLTFMaterial> materials;
    parseMaterial(materials, rootNode.get("materials").get<picojson::array>());

    // Parse scene hierarchy
    if (!rootNode.contains("nodes") ||
        !rootNode.get("nodes").is<picojson::array>())
      throw std::runtime_error("Invalid nodes");
    std::unordered_map<uint32_t, glm::mat4> meshId2Transform;
    parseSceneHierarchy(meshId2Transform,
                        rootNode.get("nodes").get<picojson::array>());

    // Parse meshes
    if (!rootNode.contains("meshes") ||
        !rootNode.get("meshes").is<picojson::array>())
      throw std::runtime_error("Invalid meshes");
...
private:
  void
  parseSceneHierarchy(std::unordered_map<uint32_t, glm::mat4> &meshId2Transform,
                      const picojson::array &nodes) const {
    ...
    for (const auto &node : nodes) {
      ...
      if (fields.find("mesh") != fields.end())
        n.mesh = static_cast<uint32_t>(fields.at("mesh").get<int64_t>());

      glm::vec3 trans(0.f);
      if (fields.find("translation") != fields.end()) {
        const auto &v3 = fields.at("translation").get<picojson::array>();
        trans = glm::vec3(v3[0].get<double>(), v3[1].get<double>(),
                          v3[2].get<double>());
      }
      ...
      if (fields.find("rotation") != fields.end()) {
        ...
        rot = glm::quat(q[3].get<double>(), q[0].get<double>(),
      ...
      if (fields.find("scale") != fields.end()) {
        ...
        scale = glm::vec3(v3[0].get<double>(), v3[1].get<double>(),
                          v3[2].get<double>());
      }

      glm::mat4 I; // identity
      n.local2parent =
          glm::translate(I, trans) * glm::mat4_cast(rot) * glm::scale(I, scale);
    ...
    std::function<void(uint32_t, glm::mat4)> visit =
        [&visit, &meshId2Transform, &ns](uint32_t root, glm::mat4 T) {
          const auto &n = ns[root];
          T *= n.local2parent;
          if (n.mesh != std::numeric_limits<uint32_t>::max())
            meshId2Transform[n.mesh] = T;
  ...
  void parseMeshes(
      std::vector<GLTFMesh> &ms, const picojson::array &meshes,
    ...
    for (uint32_t meshId = 0; meshId < meshes.size(); ++meshId) {
      ...
      glm::mat4 T = meshId2Transform.find(meshId) != meshId2Transform.end()
                        ? meshId2Transform.at(meshId)
                        : glm::mat4();
      glm::mat4 Tit = glm::transpose(glm::inverse(T));
      const auto &prims =
          mesh.get<picojson::object>().at("primitives").get<picojson::array>();

      for (const auto &prim : prims) {
        const auto &fields = prim.get<picojson::object>();
        uint32_t matId =
            static_cast<uint32_t>(fields.at("material").get<int64_t>());
        ...
        const auto &attributes =
            fields.at("attributes").get<picojson::object>();
        uint32_t posAccId =
            static_cast<uint32_t>(attributes.at("POSITION").get<int64_t>());
        uint32_t nrmAccId =
            static_cast<uint32_t>(attributes.at("NORMAL").get<int64_t>());
        uint32_t tcAccId =
            static_cast<uint32_t>(attributes.at("TEXCOORD_0").get<int64_t>());
        uint32_t idxAccId =
            static_cast<uint32_t>(fields.at("indices").get<int64_t>());
        ...
        // Positions
        {
          ...
          glm::vec3 *positions =
              reinterpret_cast<glm::vec3 *>(&m.positions[posStart]);
        ...
        // Normals
        {
          ...
          glm::vec3 *normals =
              reinterpret_cast<glm::vec3 *>(&m.normals[nrmStart]);
          for (uint32_t i = 0; i < acc.count; ++i) {
            normals[i] =
                glm::normalize(glm::vec3(Tit * glm::vec4(normals[i], 0.f)));
          }
        }
        // Texture coordinates
        {
        ...
        // Indices
        {
          ...
          while (it.hasNext()) {
            m.indices.push_back(idxOffset +
                                static_cast<uint32_t>(it.getNext()));
          }
        }
...
#define IS_VALID(x) ((x) != INVALID_VAL)
        ...
        uint32_t aoMapId = IS_VALID(material.aoTexture)
                               ? textures[material.aoTexture].source
                               : INVALID_VAL;
        uint32_t emissiveMapId = IS_VALID(material.emissiveTexture)
                                     ? textures[material.emissiveTexture].source
                                     : INVALID_VAL;
        ...
        if (IS_VALID(aoMapId))
          m.aoMap = images[aoMapId];
        if (IS_VALID(emissiveMapId))
          m.emissiveMap = images[emissiveMapId];
#undef IS_VALID
#undef INVALID_VAL
      }
    }
  ...
  void parseMaterial(std::vector<GLTFMaterial> &mats,
                     const picojson::array &materials) const {
    for (const auto &material : materials) {
      const auto &fields = material.get<picojson::object>();
      const auto &pbrTextures =
          fields.at("pbrMetallicRoughness").get<picojson::object>();

      GLTFMaterial mat;
      mat.albedoTexture =
          static_cast<uint32_t>(pbrTextures.at("baseColorTexture")
                                    .get<picojson::object>()
                                    .at("index")
                                    .get<int64_t>());
      mat.roughnessTexture =
          static_cast<uint32_t>(pbrTextures.at("metallicRoughnessTexture")
                                    .get<picojson::object>()
                                    .at("index")
                                    .get<int64_t>());
      mat.metallicTexture = mat.roughnessTexture;
      mat.normalTexture = static_cast<uint32_t>(fields.at("normalTexture")
                                                    .get<picojson::object>()
                                                    .at("index")
                                                    .get<int64_t>());
      if (fields.find("occlusionTexture") != fields.end()) {
        mat.aoTexture = static_cast<uint32_t>(fields.at("occlusionTexture")
                                                  .get<picojson::object>()
                                                  .at("index")
                                                  .get<int64_t>());
      }
      if (fields.find("emissiveTexture") != fields.end()) {
        mat.emissiveTexture = static_cast<uint32_t>(fields.at("emissiveTexture")
                                                        .get<picojson::object>()
                                                        .at("index")
                                                        .get<int64_t>());
      }
  ...
  void parseAccessors(std::vector<GLTFAccessor> &as,
                      const picojson::array &accessors) const {
    for (const auto &accessor : accessors) {
      const auto &fields = accessor.get<picojson::object>();

      GLTFAccessor a;
      a.bufferView =
          static_cast<uint32_t>(fields.at("bufferView").get<int64_t>());
      a.byteOffset =
          static_cast<uint32_t>(fields.at("byteOffset").get<int64_t>());
      a.componentType = static_cast<rj::GLTFComponentType>(
          fields.at("componentType").get<int64_t>());
  ...
  void parseBufferViews(std::vector<GLTFBufferView> &bvs,
                        const picojson::array &bufferViews) const {
    for (const auto &bufferView : bufferViews) {
      ...
      bv.byteOffset =
          static_cast<uint32_t>(fields.at("byteOffset").get<int64_t>());
      bv.byteLength =
          static_cast<uint32_t>(fields.at("byteLength").get<int64_t>());
  ...
  void parseBuffers(std::vector<GLTFBuffer> &bs, const picojson::array &buffers,
    ...
    for (const auto &buffer : buffers) {
      ...
      if (b.size() !=
          static_cast<size_t>(fields.at("byteLength").get<int64_t>()))
        throw std::runtime_error("Incorrect buffer byte length");
    }
  }

  void parseImages(std::vector<GLTFImage> &imgs, const picojson::array &images,
                   const std::string &baseDir) const {
    size_t p = imgs.size();
  ...
  void parseTextures(std::vector<GLTFTexture> &ts,
                     const picojson::array &textures) const {
